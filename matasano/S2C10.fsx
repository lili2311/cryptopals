...
//            (decryptAesEcb block iv key, block))
//        |> Seq.windowed 2
//        |> Seq.map (fun xs ->
...
//    data
//
//let encryptAesCbc (keyBytes:byte []) (iv:byte []) (bytes:byte []) =
//    let blockSize = 16
//    let blocks = chunk blockSize bytes |> Seq.toArray
//    let encryptBlock (cipherText:byte []) (plainText:byte []) : (byte []) =
//        let toEncrypt = xorBytes cipherText plainText
//        let iv = Array.zeroCreate blockSize
//        encryptAesEcb keyBytes iv toEncrypt
//    let data =
//        blocks |> Seq.scan (encryptBlock) iv
//        // 1st block result is the IV; ignore it
//        |> Seq.skip 1
